...
from src.schemas.schemas import Pedido, Usuario, Produto, ProdutoSimples
from sqlalchemy.orm import Session
from src.infra.sqlalchemy.config.database import get_db
from src.infra.sqlalchemy.repositories.repositorio_pedidos import RepositorioPedido
...
def exibir_pedido(id: int, session: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f'Não existe um pedido com id={id}'
        )
...
def listar_vendas(usuario_id: str, session: Session = Depends(get_db)):
    try:
